...
class VMUNetV2(nn.Module):
    def __init__(self,
        ...
        # SAFM (Spatially-Adaptive Feature Modulation) Module
        self.safm = SAFM(mid_channel)  # 在此处添加SAFM模块
    ...
    def forward(self, x):
        ...
        # Apply SAFM before SDI feature fusion
        f1 = self.safm(f1)  # 通过SAFM模块增强f1特征
        f2 = self.safm(f2)  # 通过SAFM模块增强f2特征
        f3 = self.safm(f3)  # 通过SAFM模块增强f3特征
        f4 = self.safm(f4)  # 通过SAFM模块增强f4特征
