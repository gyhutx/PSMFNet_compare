...
class MultiScaleInput(nn.Module):
    ...
    def forward(self, x):
        x1 = self.act(self.branch1(x))
        x2 = self.act(self.branch2(x))
        x3 = self.act(self.branch3(x))
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        # 新增多尺度分支
        self.ms_input = MultiScaleInput(hidden_dim, hidden_dim)
    ...
    def forward(self, input: torch.Tensor):
        # [B, H, W, C] -> [B, C, H, W]
        x = input.permute(0, 3, 1, 2).contiguous()
        # 多尺度特征
        x = self.ms_input(x)
        # [B, C, H, W] -> [B, H, W, C]
