...
class VMUNetV2(nn.Module):
    def __init__(self,
        ...
        self.msca = AttentionModule(dim=mid_channel)  # 新增MSCA模块
    ...
    def forward(self, x):
        ...
        # 在特征融合前应用 MSCA 提升每一层特征
        f1 = self.msca(f1)  # MSCA 处理 f1
        f2 = self.msca(f2)  # MSCA 处理 f2
        f3 = self.msca(f3)  # MSCA 处理 f3
        f4 = self.msca(f4)  # MSCA 处理 f4
