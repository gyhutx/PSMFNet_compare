...
def test_one_epoch(test_loader,
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(test_loader)):
            ...
            preds.append(out)
            if i % config.save_interval == 0:
                img_np = img.cpu().detach().numpy()  # 形状为 (batch_size, C, H, W)
                msk_np = msk.squeeze(
                    1).cpu().detach().numpy()  # 假设 msk 形状为 (batch_size, 1, H, W)，结果为 (batch_size, H, W)
                if type(model_out) is tuple:
                    for idx in range(2):
                        o = model_out[idx].squeeze(1).cpu().detach().numpy()  # 形状为 (batch_size, H, W)
                        save_dir = config.work_dir + f'outputs/out{idx}/'
                        save_imgs(img_np, msk_np, o, i, save_dir, config.datasets, config.threshold,
                                  test_data_name=test_data_name)
                    out = (model_out[0] + model_out[1]).squeeze(1).cpu().detach().numpy()  # 形状为 (batch_size, H, W)
                    save_dir = config.work_dir + 'outputs/sum/'
                else:
                    out = model_out.squeeze(1).cpu().detach().numpy()  # 形状为 (batch_size, H, W)
                    save_dir = config.work_dir + 'outputs/'
                save_imgs(img_np, msk_np, out, i, save_dir, config.datasets, config.threshold,
                          test_data_name=test_data_name)
